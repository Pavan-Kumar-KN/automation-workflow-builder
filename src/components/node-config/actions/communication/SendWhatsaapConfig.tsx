...
const SendWhatsappConfig: React.FC<ConfigComponentProps> = ({ config, setConfig }) => {
    ...
    return (
        <div className="space-y-4">
            <div>
                ...
                <p className="text-sm text-gray-500">You have to select the text type which you want to send for example plain text or template.</p>
            </div>
                    ...
                    <SelectContent>
                        {devices.map((device) => (
                            <SelectItem key={device.id} value={device.id}>
                                {device.name} ({device.phoneNumber}) - {device.status}
                            </SelectItem>
                        ))}
                    </SelectContent>
                </Select>
                {isLoadingDevices && (
                    <p className="text-xs text-blue-600 animate-pulse">Loading available devices...</p>
                )}
            </div>
...
export default SendWhatsappConfig;